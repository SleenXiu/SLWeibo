...
#define SLNewpartImageCount 3
#define SLColor(r, g, b) [UIColor colorWithRed:(r)/255.0 green:(g)/255.0 blue:(b)/255.0 alpha:1.0]
@interface SLNewPartController () <UIScrollViewDelegate>

@property (nonatomic, weak) UIPageControl *pageControl;
@end


@implementation SLNewPartController
...
- (void)setupScrollView
{
    ...
    for (int index = 0; index<SLNewpartImageCount; index++) {
        ...
        // 在最后一个图片上面添加按钮
        if (index == SLNewpartImageCount - 1) {
            [self setupLastImageView:imageView];
        }
    }
    
    // 3.设置滚动的内容尺寸
    scrollView.contentSize = CGSizeMake(imageW * SLNewpartImageCount, 0);
...
- (void)setupPageControl
{
    ...
    pageControl.numberOfPages = SLNewpartImageCount;
